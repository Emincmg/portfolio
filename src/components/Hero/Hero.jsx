...
const Hero = ({ data }) => {
  const { title, subTitle, ImgLink, phone, email, socialData, btnText } = data;
  ...
  useEffect(() => {
    ...
    const hbMeElement = document.querySelector('.hb-me');
    if (hbMeElement) {
      observer.observe(hbMeElement);
    }

    return () => {
      if (hbMeElement) {
        observer.unobserve(hbMeElement);
      }
    };
  }, []);
  ...
  return (
      <section id="home" className="home-section bg-dark">
        ...
        <div
            className="hb-me"
            style={{ backgroundImage: imageLoaded ? `url(${ImgLink})` : 'none' }}
            data-aos="fade-left"
